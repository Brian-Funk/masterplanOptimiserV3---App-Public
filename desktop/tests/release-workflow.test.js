...
test('tagged releases require the exact green public main commit and synced version', () => {
  ...
  assert.match(workflow, /\.private == false && \.visibility == "public"/);
...
test('release artefacts are bound to a keyless signed checksum manifest', () => {
  ...
  assert.match(workflow, /\.github\/workflows\/build\.yml@refs\/tags\/\$\{TAG\}/);
